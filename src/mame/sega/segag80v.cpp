...
void segag80v_state::spacfurybl(machine_config &config)
{
	...
	TMS5100(config, "tms5100", XTAL::u(640'000)).add_route(ALL_OUTPUTS, "speaker", 0.5); // clock not verified TODO: hook up
...
{
	g80v_base(config);
	SEGAUSB(config, m_usb, XTAL(), m_maincpu).add_route(ALL_OUTPUTS, "speech", 1.0, 1);
	SEGA_SPEECH_BOARD(config, "speech").add_route(ALL_OUTPUTS, "speaker", 1.0);
}
