...
void xbox_base_state::xbox_base(machine_config &config)
{
	...
	PCI_ROOT(config,        "pci");
	NV2A_HOST(config,       "pci:00.0", XTAL(), m_maincpu);
	NV2A_RAM(config,        "pci:00.3", XTAL(), 128); // 128 megabytes
	MCPX_ISALPC(config,     "pci:01.0", XTAL(), 0).interrupt_output().set(FUNC(xbox_base_state::maincpu_interrupt));
	XBOX_SUPERIO(config,    "pci:01.0:0");
	MCPX_SMBUS(config,      "pci:01.1", XTAL(), 0).interrupt_handler().set("pci:01.0", FUNC(mcpx_isalpc_device::irq11)); //.set(FUNC(xbox_base_state::smbus_interrupt_changed));
	...
	MCPX_OHCI(config,       "pci:02.0", XTAL(), 0).interrupt_handler().set("pci:01.0", FUNC(mcpx_isalpc_device::irq1));  //.set(FUNC(xbox_base_state::ohci_usb_interrupt_changed));
	MCPX_OHCI(config,       "pci:03.0", XTAL(), 0);
	MCPX_ETH(config,        "pci:04.0");
	MCPX_APU(config,        "pci:05.0", XTAL(), 0, m_maincpu);
	MCPX_AC97_AUDIO(config, "pci:06.0", XTAL(), 0);
	MCPX_AC97_MODEM(config, "pci:06.1");
	PCI_BRIDGE(config,      "pci:08.0", XTAL(), 0x10de01b8, 0);
	MCPX_IDE(config,        "pci:09.0", XTAL(), 0).pri_interrupt_handler().set("pci:01.0", FUNC(mcpx_isalpc_device::irq14));  //.set(FUNC(xbox_base_state::ide_interrupt_changed));
	NV2A_AGP(config,        "pci:1e.0", XTAL(), 0x10de01b7, 0);
	NV2A_GPU(config,        "pci:1e.0:00.0", XTAL(), m_maincpu).interrupt_handler().set("pci:01.0", FUNC(mcpx_isalpc_device::irq3)); //.set(FUNC(xbox_base_state::nv2a_interrupt_changed));
