...
void ps2sony_state::ps2sony(machine_config &config)
{
	...
	SONYPS2_TIMER(config, m_timer[0], XTAL::u(294'912'000)/2, true);
	SONYPS2_TIMER(config, m_timer[1], XTAL::u(294'912'000)/2, true);
	SONYPS2_TIMER(config, m_timer[2], XTAL::u(294'912'000)/2, false);
	SONYPS2_TIMER(config, m_timer[3], XTAL::u(294'912'000)/2, false);

	SONYPS2_INTC(config, m_intc, m_maincpu);
	SONYPS2_GS(config, m_gs, XTAL::u(294'912'000)/2, m_intc, m_vu1);
	SONYPS2_DMAC(config, m_dmac, XTAL::u(294'912'000)/2, m_maincpu, m_ram, m_sif, m_gs, m_vu1);
