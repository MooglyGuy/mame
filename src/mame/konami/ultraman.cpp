...
void ultraman_state::ultraman(machine_config &config)
{
	...
	M68000(config, m_maincpu, XTAL::u(24'000'000) / 2); // 12 MHz?
	...
	Z80(config, m_audiocpu, XTAL::u(24'000'000) / 6); // 4 MHz?
	...
	YM2151(config, "ymsnd", XTAL::u(24'000'000) / 6).add_route(0, "speaker", 1.0, 0).add_route(1, "speaker", 1.0, 1);

	okim6295_device &oki(OKIM6295(config, "oki", XTAL::u(1'056'000), okim6295_device::PIN7_HIGH)); // clock frequency & pin 7 not verified
