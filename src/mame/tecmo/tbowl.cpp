...
private:
	...
	uint16_t m_bgxscroll[2]{};
	uint16_t m_bgyscroll[2]{};
	uint16_t m_adpcm_pos[2]{};
	...
	template <uint8_t Which> void adpcm_start_w(uint8_t data);
	template <uint8_t Which> void adpcm_end_w(uint8_t data);
	template <uint8_t Which> void adpcm_vol_w(uint8_t data);
	void txvideoram_w(offs_t offset, uint8_t data);
	template <uint8_t Which> void bgvideoram_w(offs_t offset, uint8_t data);
	template <uint8_t Which> void bgxscroll_lo(uint8_t data);
	template <uint8_t Which> void bgxscroll_hi(uint8_t data);
	template <uint8_t Which> void bgyscroll_lo(uint8_t data);
	template <uint8_t Which> void bgyscroll_hi(uint8_t data);
...
{
	int tileno = m_txvideoram[tile_index] | ((m_txvideoram[tile_index + 0x800] & 0x07) << 8);
	int col = (m_txvideoram[tile_index + 0x800] & 0xf0) >> 4;
...
// Bottom BG Layer (0) and Middle BG Layer (1) Tilemaps

template <uint8_t Which>
TILE_GET_INFO_MEMBER(tbowl_state::get_bg_tile_info)
{
	int tileno = m_bgvideoram[Which][tile_index] | ((m_bgvideoram[Which][tile_index + 0x1000] & 0x0f) << 8);
	int col = (m_bgvideoram[Which][tile_index + 0x1000] & 0xf0) >> 4;

	tileinfo.set(Which + 1, Which ? tileno ^= 0x400 : tileno, col, 0);
}
...
void tbowl_state::bgvideoram_w(offs_t offset, uint8_t data)
{
	...
	m_bg_tilemap[Which]->mark_tile_dirty(offset & 0xfff);
}

template <uint8_t Which>
void tbowl_state::bgxscroll_lo(uint8_t data)
{
	m_bgxscroll[Which] = (m_bgxscroll[Which] & 0xff00) | data;
}

template <uint8_t Which>
void tbowl_state::bgxscroll_hi(uint8_t data)
{
	m_bgxscroll[Which] = (m_bgxscroll[Which] & 0x00ff) | (data << 8);
}

template <uint8_t Which>
void tbowl_state::bgyscroll_lo(uint8_t data)
{
	m_bgyscroll[Which] = (m_bgyscroll[Which] & 0xff00) | data;
}

template <uint8_t Which>
void tbowl_state::bgyscroll_hi(uint8_t data)
{
	m_bgyscroll[Which] = (m_bgyscroll[Which] & 0x00ff) | (data << 8);
}
...
void tbowl_state::video_start()
{
	...
	m_bg_tilemap[0] = &machine().tilemap().create(*m_gfxdecode, tilemap_get_info_delegate(*this, FUNC(tbowl_state::get_bg_tile_info<0>)), TILEMAP_SCAN_ROWS, 16,16, 128,32);
	m_bg_tilemap[1] = &machine().tilemap().create(*m_gfxdecode, tilemap_get_info_delegate(*this, FUNC(tbowl_state::get_bg_tile_info<1>)), TILEMAP_SCAN_ROWS, 16,16, 128,32);

	m_tx_tilemap->set_transparent_pen(0);
	m_bg_tilemap[0]->set_transparent_pen(0);
	m_bg_tilemap[1]->set_transparent_pen(0);

	save_item(NAME(m_bgxscroll));
	save_item(NAME(m_bgyscroll));
}
...
{
	m_bg_tilemap[0]->set_scrollx(0, m_bgxscroll[0]);
	m_bg_tilemap[0]->set_scrolly(0, m_bgyscroll[0]);
	m_bg_tilemap[1]->set_scrollx(0, m_bgxscroll[1]);
	m_bg_tilemap[1]->set_scrolly(0, m_bgyscroll[1]);
...
{
	m_bg_tilemap[0]->set_scrollx(0, m_bgxscroll[0] + 32*8);
	m_bg_tilemap[0]->set_scrolly(0, m_bgyscroll[0]);
	m_bg_tilemap[1]->set_scrollx(0, m_bgxscroll[1] + 32*8);
	m_bg_tilemap[1]->set_scrolly(0, m_bgyscroll[1]);
...
void tbowl_state::_6206B_map(address_map &map)
{
	...
	map(0xa000, 0xbfff).ram().w(FUNC(tbowl_state::bgvideoram_w<1>)).share(m_bgvideoram[1]);
	map(0xc000, 0xdfff).ram().w(FUNC(tbowl_state::bgvideoram_w<0>)).share(m_bgvideoram[0]);
	map(0xe000, 0xefff).ram().w(FUNC(tbowl_state::txvideoram_w)).share(m_txvideoram);
...
//  map(0xfc0a, 0xfc0a).w(FUNC(tbowl_state::unknown_write)); // hardly used ..
	...
	map(0xfc10, 0xfc10).w(FUNC(tbowl_state::bgxscroll_lo<1>));
	map(0xfc11, 0xfc11).w(FUNC(tbowl_state::bgxscroll_hi<1>));
	map(0xfc12, 0xfc12).w(FUNC(tbowl_state::bgyscroll_lo<1>));
	map(0xfc13, 0xfc13).w(FUNC(tbowl_state::bgyscroll_hi<1>));
	map(0xfc14, 0xfc14).w(FUNC(tbowl_state::bgxscroll_lo<0>));
	map(0xfc15, 0xfc15).w(FUNC(tbowl_state::bgxscroll_hi<0>));
	map(0xfc16, 0xfc16).w(FUNC(tbowl_state::bgyscroll_lo<0>));
	map(0xfc17, 0xfc17).w(FUNC(tbowl_state::bgyscroll_hi<0>));
}
...
template <uint8_t Which>
void tbowl_state::adpcm_start_w(uint8_t data)
{
	m_adpcm_pos[Which] = data << 8;
	m_msm[Which]->reset_w(0);
}

template <uint8_t Which>
void tbowl_state::adpcm_end_w(uint8_t data)
{
	m_adpcm_end[Which] = (data + 1) << 8;
}

template <uint8_t Which>
void tbowl_state::adpcm_vol_w(uint8_t data)
{
	m_msm[Which]->set_output_gain(ALL_OUTPUTS, (data & 127) / 127.0);
}
...
void tbowl_state::_6206A_map(address_map &map)
{
	...
	map(0xe000, 0xe000).w(FUNC(tbowl_state::adpcm_end_w<0>));
	map(0xe001, 0xe001).w(FUNC(tbowl_state::adpcm_end_w<1>));
	map(0xe002, 0xe002).w(FUNC(tbowl_state::adpcm_start_w<0>));
	map(0xe003, 0xe003).w(FUNC(tbowl_state::adpcm_start_w<1>));
	map(0xe004, 0xe004).w(FUNC(tbowl_state::adpcm_vol_w<0>));
	map(0xe005, 0xe005).w(FUNC(tbowl_state::adpcm_vol_w<1>));
...
static INPUT_PORTS_START( tbowl )
	...
	PORT_DIPSETTING (   0xf8, "1:00" ) // duplicate
...
NEC D70008AC-8s which is just a Z80, board a (the sound board)
has an actual Z80 chip

Sound Hardware should be 2 YM3812s + 2 MSM5205s
...
this fails its ROM check so I assume it's corrupt
