...
void tehkanwc_state::tehkanwc(machine_config &config)
{
	...
	Z80(config, m_maincpu, XTAL::u(18432000)/4);     /* 18.432000 / 4 */
	...
	ym2149_device &ay1(YM2149(config, "ay1", XTAL::u(18432000)/12));
	...
	ym2149_device &ay2(YM2149(config, "ay2", XTAL::u(18432000)/12));
...
{
	tehkanwc(config);
	ay8910_device &ay1(AY8910(config.replace(), "ay1", XTAL::u(18432000)/12));
	ay1.port_a_write_callback().set(FUNC(tehkanwc_state::portA_w));
	ay1.port_b_write_callback().set(FUNC(tehkanwc_state::portB_w));
	ay1.add_route(ALL_OUTPUTS, "mono", 0.25);

	ay8910_device &ay2(AY8910(config.replace(), "ay2", XTAL::u(18432000)/12));
	ay2.port_a_read_callback().set(FUNC(tehkanwc_state::portA_r));
	ay2.port_b_read_callback().set(FUNC(tehkanwc_state::portB_r));
	ay2.add_route(ALL_OUTPUTS, "mono", 0.25);
}
