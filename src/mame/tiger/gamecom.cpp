...
void gamecom_state::gamecom(machine_config &config)
{
	...
	DAC_8BIT_R2R(config, m_dac).add_route(ALL_OUTPUTS, "speaker", 0.5); // unknown DAC (Digital audio)
	DAC_4BIT_R2R(config, m_dac0).add_route(ALL_OUTPUTS, "speaker", 0.25); // unknown DAC (Frequency modulation)
	DAC_4BIT_R2R(config, m_dac1).add_route(ALL_OUTPUTS, "speaker", 0.25); // unknown DAC (Frequency modulation)
