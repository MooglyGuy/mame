...
public:
	...
	template <typename T> deco_karnovsprites_device(const machine_config &mconfig, const char *tag, device_t *owner, T &&palette_tag, const gfx_decode_entry *gfxinfo)
		: deco_karnovsprites_device(mconfig, tag, owner, XTAL())
	{
		set_info(gfxinfo);
