...
static GFXDECODE_START( gfx_madmotor )
	GFXDECODE_ENTRY( "char",    0, charlayout,   0, 16 ) // 8x8
	GFXDECODE_ENTRY( "tiles1",  0, tilelayout, 512, 16 ) // 16x16
	GFXDECODE_ENTRY( "tiles2",  0, tilelayout, 768, 16 ) // 16x16
GFXDECODE_END

static GFXDECODE_START( gfx_madmotor_spr )
	GFXDECODE_ENTRY( "sprites", 0, tilelayout, 256, 16 ) // 16x16
GFXDECODE_END
...
{
	bool const flip = m_tilegen[0]->get_flip_state();
...
void madmotor_state::madmotor(machine_config &config)
{
	...
	M68000(config, m_maincpu, 24_MHz_XTAL / 2); // Custom chip 59, 24 MHz crystal
	m_maincpu->set_addrmap(AS_PROGRAM, &madmotor_state::main_map);
	m_maincpu->set_vblank_int("screen", FUNC(madmotor_state::irq6_line_hold)); // VBL

	H6280(config, m_audiocpu, 8.053_MHz_XTAL / 2); // Custom chip 45, Crystal near CPU is 8.053 MHz
	...
	YM2203(config, "ym1", 21.477272_MHz_XTAL / 6).add_route(ALL_OUTPUTS, "mono", 0.40);

	ym2151_device &ym2(YM2151(config, "ym2", 21.477272_MHz_XTAL / 6));
...
ROM_START( madmotor )
	...
	ROM_LOAD16_BYTE( "04.a9",     0x000000, 0x10000, CRC(833ca3ab) SHA1(7a3e7ebecc1596d2e487595369ad9ba54ced5bfb) )
	...
	ROM_LOAD16_BYTE( "06.a13",    0x000000, 0x20000, CRC(448850e5) SHA1(6a44a42738cf6a55b4bec807e0a3939a42b36793) )
	...
	ROM_LOAD16_BYTE( "15.h11",    0x000000, 0x20000, CRC(90ae9f74) SHA1(806f96fd08fca1beeeaefe3c0fac1991410aa9c4) )
...
ROM_START( madmotora )
	ROM_REGION( 0x80000, "maincpu", 0 ) // 68000 code
	ROM_LOAD16_BYTE( "02.b4", 0x00000, 0x20000, CRC(18d3dba8) SHA1(010fefdf8325a8ccee34a43d9195ac5219698dae) )
	ROM_LOAD16_BYTE( "00.b1", 0x00001, 0x20000, CRC(686342c6) SHA1(fc568056a3a58fae8c1d4f5649f6554916ebb464) )
	ROM_LOAD16_BYTE( "03.b6", 0x40000, 0x20000, CRC(442a0a52) SHA1(86bb5470d5653d125481250f778c632371dddad8) )
	ROM_LOAD16_BYTE( "01.b3", 0x40001, 0x20000, CRC(e246876e) SHA1(648dca8bab001cfb42618081bbc1efa14118743e) )
...
GAME( 1989, madmotor,  0,        madmotor, madmotor, madmotor_state, init_madmotor, ROT0, "Mitchell", "Mad Motor (prototype, set 1)", MACHINE_SUPPORTS_SAVE )
GAME( 1989, madmotora, madmotor, madmotor, madmotor, madmotor_state, init_madmotor, ROT0, "Mitchell", "Mad Motor (prototype, set 2)", MACHINE_SUPPORTS_SAVE )
