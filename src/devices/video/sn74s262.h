...
public:
	...
	sn74s262_device(const machine_config &mconfig, const char *tag, device_t *owner, const XTAL &clock);

	u8 read(u8 character, u8 row);

protected:
	sn74s262_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, const XTAL &clock);
...
public:
	// construction/destruction
	sn74s263_device(const machine_config &mconfig, const char *tag, device_t *owner, const XTAL &clock);
