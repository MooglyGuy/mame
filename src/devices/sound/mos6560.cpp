...
void mos6560_device::sound_stream_update(sound_stream &stream)
{
	...
	for (i = 0; i < stream.samples(); i++)
	{
		...
		if (TONE1_ON /*||(m_tone1pos != 0) */ )
		{
			...
			{
				m_tone1pos = 0;
				m_tone1samples = stream.sample_rate().value() / TONE1_FREQUENCY;
				if (m_tone1samples == 0)
					m_tone1samples = 1;
			}
#endif
		...
		if (TONE2_ON /*||(m_tone2pos != 0) */ )
		{
			...
			{
				m_tone2pos = 0;
				m_tone2samples = stream.sample_rate().value() / TONE2_FREQUENCY;
				if (m_tone2samples == 0)
					m_tone2samples = 1;
			}
#endif
		...
		if (TONE3_ON /*||(m_tone3pos != 0) */ )
		{
			...
			{
				m_tone3pos = 0;
				m_tone3samples = stream.sample_rate().value() / TONE3_FREQUENCY;
				if (m_tone3samples == 0)
					m_tone3samples = 1;
			}
#endif
