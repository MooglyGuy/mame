...
void DISCRETE_CLASS_NAME(dss_input_stream)::stream_start(void)
{
	if (m_is_buffered)
	{
		...
		m_buffer_stream = m_device->machine().sound().stream_alloc(*snd_device, 0, 1, XTAL::u(sample_rate()), stream_update_delegate(&discrete_dss_input_stream_node::stream_generate,this), STREAM_DEFAULT_FLAGS);
