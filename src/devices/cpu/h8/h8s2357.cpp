...
h8s2357_device::h8s2357_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, const XTAL &clock, u32 start) :
...
bool h8s2357_device::exr_in_stack() const
{
	return m_syscr & 0x20;
}

int h8s2357_device::trace_setup()
{
	m_CCR |= F_I;
	m_EXR &= ~EXR_T;
	return 5;
...
int h8s2357_device::trapa_setup()
{
	m_CCR |= F_I;
	if(m_syscr & 0x20)
		m_EXR &= ~EXR_T;
...
void h8s2357_device::update_irq_filter()
{
	switch(m_syscr & 0x20) {
	case 0x00:
		if(m_CCR & F_I)
