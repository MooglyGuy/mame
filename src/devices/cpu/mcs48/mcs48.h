...
protected:
	...
	mcs48_cpu_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, const XTAL &clock, int rom_size, int ram_size, u8 feature_mask, const mcs48_ophandler *opcode_table);
...
public:
	// construction/destruction
	upd7751_device(const machine_config &mconfig, const char *tag, device_t *owner, const XTAL &clock);
};
