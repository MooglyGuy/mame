...
void acorn_vidc10_device::device_add_mconfig(machine_config &config)
{
	...
	{
		// custom DAC
		DAC_16BIT_R2R_TWOS_COMPLEMENT(config, m_dac[i]).add_route(0, m_speaker, m_sound_input_gain, 0).add_route(0, m_speaker, m_sound_input_gain, 1);
	}
}
...
void arm_vidc20_device::device_add_mconfig(machine_config &config)
{
	...
	DAC_16BIT_R2R_TWOS_COMPLEMENT(config, m_dac32[0]).add_route(ALL_OUTPUTS, m_speaker, 0.25, 0);
	DAC_16BIT_R2R_TWOS_COMPLEMENT(config, m_dac32[1]).add_route(ALL_OUTPUTS, m_speaker, 0.25, 1);
}
