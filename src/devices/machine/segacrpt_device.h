...
{
public:
	sega_315_5006_device(const machine_config &mconfig, const char *tag, device_t *owner, const XTAL &clock = XTAL());
protected:
	sega_315_5006_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, const XTAL &clock);
	virtual void decrypt() override;
};
