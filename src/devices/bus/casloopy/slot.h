...
public:
	// construction/destruction
	template <typename T>
	casloopy_cart_slot_device(machine_config const &mconfig, char const *tag, device_t *owner, const XTAL &clock, T &&opts, char const *dflt)
	...
	casloopy_cart_slot_device(const machine_config &mconfig, const char *tag, device_t *owner, const XTAL &clock);
