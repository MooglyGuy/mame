...
public:
	// construction/destruction
	template <typename T>
	nes_aladdin_slot_device(const machine_config &mconfig, const char *tag, device_t *owner, const XTAL &clock, T &&opts)
		: nes_aladdin_slot_device(mconfig, tag, owner, clock)
	{
		option_reset();
		opts(*this);
