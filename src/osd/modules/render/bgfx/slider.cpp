...
const bgfx_slider::string_to_enum bgfx_slider::TYPE_NAMES[bgfx_slider::TYPE_COUNT] = {
...
const bgfx_slider::string_to_enum bgfx_slider::SCREEN_NAMES[bgfx_slider::SCREEN_COUNT] = {
...
std::vector<bgfx_slider*> bgfx_slider::from_json(const Value& value, const std::string &prefix, effect_manager& effects, uint32_t screen_index)
{
	...
	std::string prefixed_desc = util::string_format("Window %1$u, Screen %2$u, %3$s", effects.window_index(), screen_index, description);
	if (slider_count > 1)
	{
		...
		for (int index = 0; index < slider_count; index++)
		{
			...
			sliders.push_back(new bgfx_slider(effects.machine(), std::move(full_name), min[index], defaults[index], max[index], step, type, screen_type, format, desc, strings));
		}
	}
	else
	{
		...
		sliders.push_back(new bgfx_slider(effects.machine(), name + "0", min, def, max, step, type, screen_type, format, prefixed_desc, strings));
	}
	return sliders;
}
...
bool bgfx_slider::get_values(const Value& value, const std::string &prefix, const std::string &name, float* values, const int count)
{
...
bool bgfx_slider::validate_parameters(const Value& value, const std::string &prefix)
{
