...
public:
	...
	effect_manager(running_machine& machine, const osd_options& options, texture_manager& textures, target_manager& targets, shader_manager& shaders, uint32_t window_index,
	...
	uint32_t process_screen_effects(uint32_t view, osd_window& window);
	...
	shader_manager& shaders() const { return m_shaders; }
	...
	bgfx_effect* screen_effect(uint32_t screen);
	std::unique_ptr<bgfx_effect> load_effect(std::string name, uint32_t screen_index);
	bool has_applicable_effect(uint32_t screen);
...
private:
	...
	{
	public:
		effect_desc(const effect_desc &) = default;
		effect_desc(effect_desc &&) = default;
		effect_desc &operator=(const effect_desc &) = default;
		effect_desc &operator=(effect_desc &&) = default;

		effect_desc(std::string &&name, std::string &&path)
	...
	void get_default_effect_info(std::string &out_effect_name, int32_t &out_effect_index);
	void refresh_available_effects();
	void destroy_unloaded_effects();
	void find_available_effects(std::string_view root, std::string_view path);
	void parse_effect_selections(std::string_view effect_str);
	std::vector<std::string_view> split_option_string(std::string_view effect_str) const;

	void update_screen_count(uint32_t screen_count);

	void set_current_effect(uint32_t screen, int32_t effect_index);
	...
	int32_t                     m_default_effect_index;
	std::vector<effect_desc>    m_available_effects;
	std::vector<bgfx_effect*>   m_screen_effects;
	std::vector<std::string>    m_effect_names;
	std::vector<ui::menu_item>  m_selection_sliders;
	std::vector<std::unique_ptr<slider_state>> m_core_sliders;
	std::vector<int32_t>        m_current_effect;
	std::vector<bgfx_texture*>  m_screen_textures;
	std::vector<bgfx_texture*>  m_screen_palettes;
	std::vector<bgfx_shader*>   m_converters;
	bgfx_shader *               m_adjuster;
	std::vector<screen_prim>    m_screen_prims;
	std::vector<uint8_t>        m_palette_temp;

	static inline constexpr uint32_t EFFECT_NONE = 0;
};
