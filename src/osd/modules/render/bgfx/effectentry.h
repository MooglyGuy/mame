...
//============================================================
//
//  effectentry.h - BGFX shader post-processing node
//
...
#ifndef DRAWBGFX_EFFECT_ENTRY
#define DRAWBGFX_EFFECT_ENTRY
...
class bgfx_effect_entry : public state_reader
{
public:
	bgfx_effect_entry(std::string name, bgfx_shader* shader, clear_state* clear, std::vector<bgfx_suppressor*> suppressors, std::vector<bgfx_input_pair*> inputs,
		std::vector<bgfx_entry_uniform*> uniforms, target_manager& targets, std::string output, bool apply_tint);
	~bgfx_effect_entry();

	static bgfx_effect_entry* from_json(const Value& value, const std::string &prefix, effect_manager& effects, std::map<std::string, bgfx_slider*>& sliders, std::map<std::string, bgfx_parameter*>& params, uint32_t screen_index);

	void submit(int view, effect_manager::screen_prim &prim, texture_manager& textures, uint16_t screen_count, uint16_t screen_width, uint16_t screen_height,
...
private:
	...
	void setup_auto_uniforms(effect_manager::screen_prim &prim, texture_manager& textures, uint16_t screen_count, uint16_t view_width, uint16_t view_height,
	...
	void setup_sourcesize_uniform(effect_manager::screen_prim &prim) const;
	...
	void setup_quaddims_uniform(effect_manager::screen_prim &prim) const;
